...
def create_world_state(system):
    ...
    system['env'].add_action("Human", "(?u - agent)", "(and (collected backpack) (collected compass) (collected water_bottle) (not(outside ?u)) )", "(outside ?u)", "hiking")
    system['env'].add_action("Human", "(?u - agent)", "(and (collected walking_stick) (collected dog) (collected water_bottle) (not(outside ?u)) )", "(outside ?u)", "promenade")
    system['env'].add_action("Human", "(?u - agent)", "(and (not (outside ?u)) (collected water_bottle) (collected sugar) (collected tea) (collected milk) )", "(not (outside ?u))", "reading_book")
    system['env'].add_action("Human", "(?u - agent)", "(and (not (outside ?u)) (collected sugar) (collected chocolate) (collected milk) (collected flour) )", "(not (outside ?u))", "watching_tv")
...
#    system['env'].add_action("Robot", "( )", "(current_weather rain)", "(collected umbrella)", "warn_rain")
    ...
    #hiking
    system['env'].add_goal('(and (collected backpack) (collected compass) (collected water_bottle))')
    #promenade
    system['env'].add_goal('(and (collected walking_stick) (collected dog) (collected water_bottle))')
    #watching_tv
    system['env'].add_goal('(and (collected water_bottle) (collected sugar) (collected tea) (collected milk))')
    # Baking Cake
    system['env'].add_goal('(and (collected sugar) (collected chocolate) (collected milk) (collected flour))')
...
def updateSituation(system):
    ...
    return opp_eqm, oop_intent, evolve_map, react, des, intent_map, K
